-- ============================================================================
-- SICOM-ICEO | 169 — Reporte fiabilidad: lugar físico SIN faena (coherencia)
-- ----------------------------------------------------------------------------
-- La faena estructurada (ej. "Andina — Boart Longyear") quedaba obsoleta porque
-- el cambio de estado en Sugerencias GPS no la actualiza (sí actualiza operación
-- y ubicación). Eso hacía que el informe mostrara "Andina" mientras Sugerencias
-- mostraba Calama / Spence. Decisión: NO usar faena; el lugar físico = la
-- ubicación libre (activos.ubicacion_actual) y la zona = operación. Así todo
-- queda coherente con lo que se edita en Sugerencias.
--
-- Cambios vs MIG 167:
--   - 'lugar_fisico' deja de concatenar la faena → usa solo la ubicación.
--   - 'faena' se devuelve NULL (se deja de mostrar en el frontend).
  ...
  SELECT jsonb_build_object(
    ...
    'equipos', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        -- Faena se descontinúa (quedaba obsoleta). Lugar físico = ubicación libre.
        'faena', NULL,
        'ubicacion', a.ubicacion_actual,
        'lugar_fisico', NULLIF(a.ubicacion_actual, ''),
        -- Zona = operación (Calama / Coquimbo / ...)
        'zona', a.operacion,
...
BEGIN
    ...
    RAISE NOTICE 'equipos: % (faena ya no se usa; lugar_fisico = ubicacion)',
