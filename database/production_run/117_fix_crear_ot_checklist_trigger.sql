-- ============================================================================
-- SICOM-ICEO | 117 — Fix rpc_crear_ot: no duplicar checklist (lo pone el trigger)
-- ============================================================================
-- Causa real: el trigger trg_copiar_checklist_para_ot copia el checklist del
-- template al INSERT de la OT. rpc_crear_ot ADEMÁS insertaba checklist_ot →
-- duplicaba (violaba UNIQUE(ot_id, orden)) y bloqueaba toda creación de OT.
--
-- Fix: rpc_crear_ot ya NO inserta el template (lo hace el trigger). Solo cuando
-- la OT viene de un PLAN, reemplaza el checklist genérico por el de la pauta.
-- (Mantiene el permiso de 'fuera_servicio'.)
-- ============================================================================
...
BEGIN
    ...
    IF p_plan_mantenimiento_id IS NOT NULL THEN
        ...
        IF v_pauta_items IS NOT NULL AND jsonb_array_length(v_pauta_items) > 0 THEN
            ...
            SELECT gen_random_uuid(), v_ot_id, t.ord::INTEGER, t.item->>'descripcion',
...
DO $$ BEGIN RAISE NOTICE 'MIG117 OK: rpc_crear_ot ya no duplica checklist'; END $$;
