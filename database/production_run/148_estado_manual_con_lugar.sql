-- ============================================================================
-- SICOM-ICEO | 148 — Cambio de estado manual captura el LUGAR FISICO
-- ----------------------------------------------------------------------------
-- En la sugerencia de estado (GPS) / modal de cambio de estado, ademas del
-- contrato se escribe el LUGAR FISICO en texto libre (ej. Salvador,
-- Chuquicamata). Este RPC ahora acepta p_ubicacion y la fija en
-- activos.ubicacion_actual ANTES de cambiar el estado_comercial, para que el
-- trigger del historial (MIG 145) capture el lugar correcto en el snapshot.
--
-- CREATE OR REPLACE: identica a la version viva + el nuevo parametro opcional.
-- Como agregar un parametro crea un OVERLOAD nuevo (no reemplaza), primero se
-- elimina la firma antigua de 9 args para evitar ambiguedad en PostgREST.
-- IDEMPOTENTE.
-- ============================================================================

-- Eliminar la firma antigua (sin p_ubicacion) si existe.
DROP FUNCTION IF EXISTS public.rpc_actualizar_estado_diario_manual(
    uuid, date, character, text, boolean, tipo_ot_enum, prioridad_enum, uuid, text);
...
CREATE OR REPLACE FUNCTION public.rpc_actualizar_estado_diario_manual(
    ...
    p_ubicacion        varchar DEFAULT NULL   -- NUEVO: lugar fisico (texto libre)
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- 2b. LUGAR FÍSICO (texto libre) ANTES del cambio de estado, para que el
    --     trigger del historial capture el snapshot correcto.
