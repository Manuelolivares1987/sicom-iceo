-- ============================================================================
-- 07_apply_mig56_fifo_produccion.sql  —  FIFO repuestos. PRODUCCION.
-- ----------------------------------------------------------------------------
-- DEPENDE DE: 04 (mig 55) validada.
-- IDEMPOTENTE.
-- ============================================================================

-- Verificar que mig 55 está aplicada
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables
                    WHERE table_schema='public' AND table_name='salidas_bodega_items') THEN
        RAISE EXCEPTION 'STOP — mig 55 no aplicada. Ejecutar primero el paso 04.';
    END IF;
END $$;

-- Log inicio
SELECT fn_log_operacion_migracion('PROD_MIG56_START', 'Iniciando aplicacion FIFO repuestos', 'pendiente', NULL);
...
CREATE TABLE IF NOT EXISTS inventario_capas (
    ...
    costo_total_inicial NUMERIC(16,2) GENERATED ALWAYS AS (cantidad_inicial * costo_unitario) STORED,
    costo_total_disponible NUMERIC(16,2) GENERATED ALWAYS AS (cantidad_disponible * costo_unitario) STORED,
    lote VARCHAR(60),
    vencimiento DATE,
...
DROP TRIGGER IF EXISTS trg_capa_updated_at ON inventario_capas;
...
CREATE TABLE IF NOT EXISTS inventario_consumos_capas (
    ...
    costo_total_consumido NUMERIC(16,2) GENERATED ALWAYS AS (cantidad_consumida * costo_unitario_capa) STORED,
...
BEGIN
    ...
    IF v_total_disponible < p_cantidad THEN
        RAISE EXCEPTION 'Stock insuficiente para producto % en bodega %. Disponible: %, solicitado: %',
            p_producto_id, p_bodega_id, v_total_disponible, p_cantidad USING ERRCODE='P0001';
    END IF;
    FOR v_capa IN
...
SELECT
    ...
    b.id AS bodega_id, b.codigo AS bodega_codigo,
    SUM(ic.cantidad_disponible) AS cantidad_total_disponible,
    SUM(ic.cantidad_disponible * ic.costo_unitario) AS valor_total_fifo,
    ROUND(SUM(ic.cantidad_disponible * ic.costo_unitario) / NULLIF(SUM(ic.cantidad_disponible), 0), 4)
        AS costo_promedio_informativo,
...
-- Verificación
SELECT 'TABLAS_FIFO' AS check_name, COUNT(*) FROM information_schema.tables
...
-- Log fin
SELECT fn_log_operacion_migracion('PROD_MIG56_END', 'Mig 56 FIFO aplicada.', 'ok',
    'Continuar con paso 08_validate. Capas iniciales en paso 09 (manual).');


-- ============================================================================
-- ROLLBACK MANUAL
-- DROP VIEW v_stock_valorizado_fifo;
-- DROP FUNCTION fn_consumir_inventario_fifo CASCADE;
-- DROP TABLE inventario_consumos_capas CASCADE;
-- DROP TABLE inventario_capas CASCADE;
-- ALTER TABLE ot_materiales_planeados
--   DROP COLUMN costo_unitario_real, DROP COLUMN costo_total_real,
--   DROP COLUMN metodo_costeo, DROP COLUMN salida_bodega_id, DROP COLUMN ceco_id;
-- ============================================================================
