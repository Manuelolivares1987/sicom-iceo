-- ============================================================================
-- 70_fix_salida_tipo_movimiento.sql
-- ----------------------------------------------------------------------------
-- FIX: MIG66/MIG69 mapeaban p_destino_tipo a tipo_movimiento='salida_consumo',
-- valor que NO existe en el CHECK constraint del kardex. Valores permitidos:
--   stock_inicial, ingreso_compra, salida_venta, salida_equipo,
--   salida_despacho, ajuste, varillaje
--
-- Error en runtime:
--   new row violates check constraint combustible_kardex_valorizado_tipo_movimiento_check
--
-- Mapeo correcto:
--   destino_tipo='venta_externa'      -> 'salida_venta'
--   destino_tipo='equipo'              -> 'salida_equipo'
--   destino_tipo en (ot,ceco,faena,
--                    consumo_interno) -> 'salida_despacho'
--
-- ADITIVA, IDEMPOTENTE. Dropea todas las signatures previas y recrea.
-- ============================================================================

-- ── DROP de signatures previas ─────────────────────────────────────────────
DO $$
DECLARE
    r RECORD;
BEGIN
...
BEGIN
    ...
    -- MIG70: mapeo correcto contra el CHECK constraint del kardex
    v_tipo_kardex := CASE p_destino_tipo
                       WHEN 'venta_externa' THEN 'salida_venta'
                       WHEN 'equipo'        THEN 'salida_equipo'
                       ELSE                       'salida_despacho'  -- ot, ceco, faena, consumo_interno
                     END;
...
'Salida valorizada CPP vigente. MIG70: tipo_movimiento mapea contra check constraint (salida_venta/equipo/despacho).';
...
SELECT jsonb_build_object(
    'rpc_actualizado', EXISTS(
        SELECT 1 FROM pg_proc p
         WHERE p.proname='rpc_registrar_salida_combustible_valorizada'
    ),
    'una_sola_signature', (
        SELECT COUNT(*) FROM pg_proc
         WHERE proname='rpc_registrar_salida_combustible_valorizada'
    ) = 1
) AS resultado;
