-- ============================================================================
-- SICOM-ICEO | 167 — Reporte fiabilidad: agregar ZONA (operación) por equipo
-- ----------------------------------------------------------------------------
-- El Excel del análisis de fiabilidad necesita la ZONA del equipo (campo
-- activos.operacion, etiquetado "Operación / zona" — Calama / Coquimbo / ...).
-- Se agrega 'zona' al objeto de cada equipo. El desglose de días por estado
-- (letras A..F) y su cuadre con UP/DOWN se calcula en el frontend desde la
-- matriz (que ya viene en este mismo payload), por lo que NO se toca aquí.
--
-- CREATE OR REPLACE idéntica a la versión viva (MIG 149) + el campo 'zona'.
-- IDEMPOTENTE.
-- ============================================================================
...
BEGIN
    ...
    RAISE NOTICE 'equipos: % | con zona: %',
        jsonb_array_length(v->'equipos'),
        (SELECT count(*) FROM jsonb_array_elements(v->'equipos') e WHERE e->>'zona' IS NOT NULL);
END $$;
