-- ============================================================================
-- 03_apply_mig55_bodega_combustible_base.sql
-- ----------------------------------------------------------------------------
-- Aplicacion staging de la migracion 55 (bodega + combustible base).
-- Versión EJECUTABLE — extraída de la mig 55 con descomentado controlado
-- y simplificaciones de las RPCs para staging (versiones base sin role-check
-- estricto, listas para reescribirse en mig 56 con FIFO).
--
-- IDEMPOTENTE: usa CREATE TYPE/TABLE/INDEX IF NOT EXISTS y ON CONFLICT.
--
-- DEPENDE DE: mig 01-51 aplicadas (verificar con 01_prechecks_safe.sql).
-- POBLAR DESPUES con: 02_seed_datos_maestros.sql
-- ============================================================================
...
SELECT 'TABLAS_NUEVAS_CREADAS' AS check_name,
...
SELECT 'FUNCIONES_FOLIO' AS check_name,
       COUNT(*) AS encontradas
  FROM pg_proc
 WHERE proname LIKE 'fn_generar_folio_%';
-- Esperado: 5
