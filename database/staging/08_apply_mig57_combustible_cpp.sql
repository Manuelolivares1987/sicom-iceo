-- ============================================================================
-- 08_apply_mig57_combustible_cpp.sql  —  Combustible CPP movil + kardex.
-- ----------------------------------------------------------------------------
...
-- Indice unico parcial: solo una partida activa por estanque.
DROP INDEX IF EXISTS uq_stock_inicial_activo;
CREATE UNIQUE INDEX uq_stock_inicial_activo
    ON combustible_stock_inicial (estanque_id)
    WHERE anulado = false;
...
-- ── 4. Extender ingresos_combustible y salidas_combustible ───────────
...
BEGIN
    ...
    IF v_rol NOT IN ('administrador','subgerente_operaciones') THEN
        RAISE EXCEPTION 'Rol % no autorizado para stock inicial', v_rol;
    END IF;
    ...
    IF p_observacion IS NULL OR LENGTH(TRIM(p_observacion)) < 5 THEN
        RAISE EXCEPTION 'Observacion obligatoria (min 5 caracteres)';
    END IF;
    ...
    IF v_activo_existente IS NOT NULL THEN
        RAISE EXCEPTION 'Ya existe stock inicial activo (id=%). Anule antes de crear nuevo.', v_activo_existente;
    END IF;
...
SELECT 'TABLAS_57' AS check_name,
...
SELECT 'COLUMNAS_ESTANQUES_CPP' AS check_name,
...
SELECT 'RPC_STOCK_INICIAL' AS check_name, COUNT(*)
